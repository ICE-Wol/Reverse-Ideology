    ...
    public class StarRingController : MonoBehaviour
    {
        ...
        private void Start() {
            ...
            for (int i = 0; i < num * 4; i++) {
                particles[i] = ParticleManager.Manager.GetParticle(sprite);
                ...
                if (rotation != 0) {
                    particles[i].SetColor(Calc.SetAlpha(Color.yellow, 0.5f));
                }
            }

            glows = new Particle[num * 4];
            for (int i = 0; i < num * 4; i++) {
                glows[i] = ParticleManager.Manager.GetParticle(sprite);
                glows[i].transform.SetParent(this.transform);
                if (i % 2 == 0) {
                    if(type == ItemType.LifeFrag || type == ItemType.Life)
                        glows[i].SetColor(Color.magenta);
                    if(type == ItemType.BombFrag || type == ItemType.Bomb)
                        glows[i].SetColor(Color.green);
                ...
                glows[i].spriteRenderer.material = BulletManager.Manager.GetBulletMaterial(true);
                particles[i].transform.localPosition = Vector3.zero;
            }
        }
        ...
        private void FixedUpdate() {
            ...
            for (int i = 0; i < num * 4; i++) {
                _positions[i] = rad1 * (Vector3)Calc.Deg2Dir(360f * i / (num * 4) + _timer);
                if (i % 2 != 0) {
                    _positions[i] = rad2 * (Vector3)Calc.Deg2Dir(360f * i / (num * 4) + _timer);
                }
            ...
            if(rotation == 0){
                ...
                transform.localRotation = Quaternion.Euler(90 * Mathf.Sin(_timer / 4f * Mathf.Deg2Rad),_timer / 2f,90);
            }
            else {
                transform.localRotation = Quaternion.Euler(90 * Mathf.Cos(_timer / 4f * Mathf.Deg2Rad),_timer / 2f,90);
            }
